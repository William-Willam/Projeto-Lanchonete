...
// Luis Eduardo: Sua tarefa é melhorar o código abaixo.
...
// E deixar a nota fiscal mais bonita, com bordas e espaçamento adequado.
    ...
    const char *formaPagamento)
{
    ...
    fprintf(arquivo, "|                     NOTA FISCAL DIGITAL                       |\n");
    fprintf(arquivo, "+---------------------------------------------------------------+\n");

    // Dados do cliente
    fprintf(arquivo, "| Cliente : %-50s |\n", cliente.nome);
    fprintf(arquivo, "| CPF     : %-50s |\n", cliente.CPF);
    fprintf(arquivo, "| Telefone: %-50s |\n", cliente.telefone);
    fprintf(arquivo, "+---------------------------------------------------------------+\n");

    // Tabela de produtos
    fprintf(arquivo, "| %-30s | %5s | %12s |\n", "Produto", "Qtd", "Subtotal");
    fprintf(arquivo, "+---------------------------------------------------------------+\n");

    for (int i = 0; i < qtdPedidos; i++)
    {
        float sub = pedidos[i].produto.preco * pedidos[i].quantidade;
        fprintf(arquivo, "| %-30s | %5d | R$ %9.2f |\n",
